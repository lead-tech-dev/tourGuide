...
/**
 * RewardsServiceImpl. class that implement
...
public class RewardsServiceImpl implements RewardsService {
      ...
      for (int i = 0; i < userLocations.size(); i++) {
        for (Attraction attraction : attractions) {
          ...
          if (user.getUserRewards().stream().noneMatch(
                  r -> r.getAttraction().attractionName.equals(
            ...
            if (nearAttraction(userLocations.get(i), attraction)) {
              ...
              user.addUserReward(new UserReward(userLocations.get(i),
                      attraction, rewardPoint));
            }
          }
        }
      }
   ...
   */
  @Override
  public Map<Double, Object> getAttractionDistance(
          List<Attraction> attractions, Location location) {
  ...
  public CompletableFuture<Integer> getRewardPoints(Attraction attraction, User user) {
    return CompletableFuture.completedFuture(
            rewardsRepository.getAttractionRewardPoints(attraction.attractionId,
            user.getUserId()));
  }
