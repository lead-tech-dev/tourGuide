...
public class UserRepository {
  ...
  public User findById(UUID userId) {
    Optional<User> user = internalUserMap.values().stream().filter(current -> current.getUserId() == userId).findFirst();
  ...
  private void initializeInternalUsers() {
    IntStream.range(0, InternalTestHelper.getInternalUserNumber()).forEach(i -> {
      ...
      User user = new User(UUID.randomUUID(), userName, phone, email, new UserPreferences(getRandomNumber(1, 10), getRandomNumber(1, 10)));
